...
if(empty($filters)) {
	...
	<div class="sixteen columns">
		...
		<!-- Page Title -->
		<div id="page-title">
			<h3><?php $pf_title = get_post_meta($post->ID, 'centum_portfolio_title', true); if($pf_title) { echo $pf_title;} else { echo ot_get_option('incr_portfolio_page'); } ?> <?php if(is_tax()) { $term = get_term_by( 'slug', get_query_var( 'term' ), get_query_var( 'taxonomy' ) );  if($term) echo '<span>/ '.$term->name.'</span>'; } ?></h3>
